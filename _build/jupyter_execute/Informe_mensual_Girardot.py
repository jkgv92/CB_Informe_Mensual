...
def post_process_data(df):
    ...
    df['festivo'] = pd.to_datetime(df.index).week
...
LST_DEVICE_ID_TO_REQUEST = '61b74fc748879b4539e30124'
...
PICKLED_DATA_FILENAME = 'parsed_response_Girardot.pkl'
...
past_months = subset_resample(
    df_bl[df_bl["variable"] == 'front-consumo-activa'], 
    agg_func=assemble_aggregator(list(df_bl.columns), np.sum, 'last'), 
    subset='device',
    freq='1M'
)

past_hour = subset_resample(
    df_bl[df_bl["variable"] == 'front-consumo-activa'], 
    agg_func=assemble_aggregator(list(df_bl.columns), np.sum, 'last'), 
    subset='device',
    freq='1h'
...
cargas_month = subset_resample(
    cargas, 
    agg_func=assemble_aggregator(list(cargas.columns), np.sum, 'last'), 
    subset='device',
    freq='1M'
)

cargas_day = subset_resample(
    cargas, 
    agg_func=assemble_aggregator(list(cargas.columns), np.sum, 'last'), 
    subset='device',
    freq='1D'
...
cargas_pot_hour = subset_resample(
    cargas_pot, 
    agg_func=assemble_aggregator(list(cargas.columns), np.mean, 'last'), 
    subset='device',
    freq='1h'
)

front_hour = subset_resample(
    front, 
    agg_func=assemble_aggregator(list(front.columns), np.sum, 'last'), 
    subset='device',
    freq='1h'
)

front_month = subset_resample(
    front, 
    agg_func=assemble_aggregator(list(front.columns), np.sum, 'last'), 
    subset='device',
    freq='1M'
...
front_reactiva_hour = subset_resample(
    front_reactiva, 
    agg_func=assemble_aggregator(list(front.columns), np.sum, 'last'), 
    subset='device',
    freq='1h'
...
Cargas_Nocturne_day = subset_resample(
    cargas_nocturne,
    agg_func=assemble_aggregator(list(front.columns), np.sum, 'last'), 
    subset='device',
    freq='1D'
...
r = front_reactiva_hour[["day","hour", "value"]]
...
sns.lineplot(
        ...
        hue=None,
        linestyle="dashed",
        data=front_reactiva_hour, 
        ci=None,
        legend=False,
        # palette="flare",
        label="Frontera reactiva [kVArh]"
    )
