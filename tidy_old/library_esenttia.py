...
def plot_typical_day_by_hour(df_subset, title=None, include_ci=False, fill_ci=True):
    idx = 0

    fig = go.Figure()
    for year in set(df_subset['year']):
        df_plot = df_subset.query("year == @year")
...
def plot_typical_week_by_day(df_subset, title=None, include_ci=False, fill_ci=True):
    idx = 0
...
def plot_typical_year_by_week(df_subset, title=None, include_ci=False, fill_ci=True):
    idx = 0
